...
DAYS_IN_WEEK = 7
SATURDAY_DOW = 5  # Day of week, for use with `date.weekday()` function, which returns 0 (Mon) - 6 (Sun)
...
def prev_saturday(limited_d):
    return limited_d - timedelta(days=((limited_d.weekday() + (DAYS_IN_WEEK - SATURDAY_DOW)) % DAYS_IN_WEEK))
...
log('Requested dates: {} to {}'.format(format_ymd(start_date_in), format_ymd(end_date_in)))

start_date, end_date = prev_saturday(start_date_in), prev_saturday(end_date_in)
log('Converted dates: {} to {}'.format(format_ymd(start_date), format_ymd(end_date)))
...
# 'pos_peak' (int): Peak position of the song across all chart weeks. Always present and non-0.
